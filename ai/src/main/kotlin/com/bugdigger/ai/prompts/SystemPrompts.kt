...
object SystemPrompts {
    ...
    val REVERSE_ENGINEERING: String = """
        ...
        - find_cross_references - find method callers and class users
        ...
        (C) Cross-reference analysis across multiple classes
          1. Use get_class_info to get exact method descriptors, then call
             find_cross_references for callers of important methods and users of
             important classes.
          2. Identify the cluster from those xrefs: callers, constructed types,
             field types, and return values. A bounded BFS of 2-6 classes is usually enough.
          3. Use decompile_classes to pull them together in one response.
          4. Read the classes as a flow: who constructs whom, what state moves where.
             Runtime evidence from get_recent_traces - if available - makes the
             call order concrete.
          5. Produce a short narrative of the flow, then apply renames with
